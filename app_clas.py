...
st.set_page_config("SVM Classification", layout="wide")

def load_css(path):
    with open(path) as f:
        st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True)

load_css("style_cla.css")

st.markdown('<div class="app-title">SVM Classification Dashboard</div>', unsafe_allow_html=True)
st.markdown('<div class="app-subtitle">End-to-End ML Pipeline with Valid Target Selection</div>', unsafe_allow_html=True)
...
def get_valid_classification_targets(df, max_unique=20):
    valid = []
    for col in df.columns:
        if df[col].nunique() >= 2 and df[col].nunique() <= max_unique:
            valid.append(col)
    return valid
...
if source == "Upload CSV Dataset":
    ...
    if uploaded is not None:
        ...
        df.to_csv(os.path.join(RAW_DIR, uploaded.name), index=False)
...
if 'df_model' in locals():
    st.markdown('<div class="section-header">Step 6: Train SVM Classification</div>', unsafe_allow_html=True)

    valid_targets = get_valid_classification_targets(df_model)

    if not valid_targets:
        st.error("No valid classification target found in dataset")
        st.stop()

    st.info(
        "Target columns are filtered automatically "
        "(categorical-like columns only)."
    )
